...
module.exports = {
	...
	onStart: async ({ args, message, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, commandName, event, getLang }) => {
		const { configCommands } = global.GoatBot;
		const { log, loadScripts } = global.utils;

		if (args[0] == "load" && args.length == 2) {
			if (!args[1])
				return bot.sendMessage(chatId, getLang("missingFileName"));
			const infoLoad = loadScripts("events", args[1], log, configCommands, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang);
			infoLoad.status == "success" ?
				bot.sendMessage(chatId, getLang("loaded", infoLoad.name)) :
				bot.sendMessage(chatId, getLang("loadedError", infoLoad.name, infoLoad.error, infoLoad.message));
		}
			...
			for (const fileName of allFile) {
				const infoLoad = loadScripts("events", fileName, log, configCommands, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang);
				infoLoad.status == "success" ?
			...
			if (!args[1])
				return bot.sendMessage(chatId, getLang("missingCommandNameUnload"));
			const infoUnload = global.utils.unloadScripts("events", args[1], configCommands, getLang);
			infoUnload.status == "success" ?
				bot.sendMessage(chatId, getLang("unloaded", infoUnload.name)) :
				bot.sendMessage(chatId, getLang("unloadedError", infoUnload.name, infoUnload.error.name, infoUnload.error.message));
		}
			...
			if (!url || !fileName)
				return bot.sendMessage(chatId, getLang("missingUrlCodeOrFileName"));
			...
			if (url.match(/(https?:\/\/(?:www\.|(?!www)))/)) {
				if (!fileName || !fileName.endsWith(".js"))
					return bot.sendMessage(chatId, getLang("missingFileNameInstall"));

				const domain = getDomain(url);
				if (!domain)
					return bot.sendMessage(chatId, getLang("invalidUrl"));
			...
			if (!rawCode)
				return bot.sendMessage(chatId, getLang("invalidUrlOrCode"));
			if (fs.existsSync(path.join(__dirname, "..", "events", fileName)))
				return bot.sendMessage(chatId, getLang("alreadExist"), (err, info) => {
				...
				const infoLoad = loadScripts("events", fileName, log, configCommands, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode);
				infoLoad.status == "success" ?
					bot.sendMessage(chatId, getLang("installed", infoLoad.name, path.join(__dirname, fileName).replace(process.cwd(), ""))) :
					bot.sendMessage(chatId, getLang("installedError", infoLoad.name, infoLoad.error.name, infoLoad.error.message));
			}
		}
		else
			message.SyntaxError();
	...
	onReaction: async function ({ Reaction, message, event, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang }) {
		const { author, messageID, data: { fileName, rawCode } } = Reaction;
		if (event.userID != author)
			return;
		const { configCommands } = global.GoatBot;
		const { log, loadScripts } = global.utils;
		const infoLoad = loadScripts("cmds", fileName, log, configCommands, bot, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode);
		infoLoad.status == "success" ?
			bot.sendMessage(chatId, getLang("installed", infoLoad.name, path.join(__dirname, '..', 'events', fileName).replace(process.cwd(), ""), () => message.unsend(messageID))) :
			bot.sendMessage(chatId, getLang("installedError", infoLoad.name, infoLoad.error.name, infoLoad.error.message, () => message.unsend(messageID)));
	}
};