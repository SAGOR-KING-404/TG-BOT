...
module.exports = {
		...
		switch (args[0]) {
			case 'add': {
				...
				if (!key || !content && attachments.length === 0)
					return bot.sendMessage(chatId, getLang('missingContent'));
				...
				if (alreadyExists) {
					if (alreadyExists.author == senderID)
						return bot.sendMessage(chatId, getLang('shortcutExists', key), async (err, info) => {
							if (err)
								return;
						...
						return bot.sendMessage(chatId, getLang('shortcutExistsByOther'));
				}
			...
			case 'list': {
				...
				if (args[1]) {
					...
					if (shortCutList.length === 0) {
						if (type == "start")
							return bot.sendMessage(chatId, getLang('listWithTypeStartNot', keyword));
						else if (type == "end")
							return bot.sendMessage(chatId, getLang('listWithTypeEndNot', keyword));
						else
							return bot.sendMessage(chatId, getLang('listWithTypeContainNot', keyword));
					}
				}
				...
				bot.sendMessage(chatId, stringType + '\n' + list);
				break;
			}
			case 'remove':
			case '-rm':
			...
			case 'rm': {
				if (threadID != senderID && role < 1)
					return bot.sendMessage(chatId, getLang('onlyAdminRemoveAll'));
				bot.sendMessage(chatId, getLang('confirmRemoveAll'), (err, info) => {
					if (err)
						return;
					global.GoatBot.onReaction.set(info.messageID, {
						commandName,
	...
	onReaction: async function ({ event, message, threadsData, getLang, Reaction }) {
		...
		if (Reaction.type == 'removeAll') {
			...
			return bot.sendMessage(chatId, getLang('removedAll'));
		}
			...
			await threadsData.set(threadID, shortCutData, 'data.shortcut');
			return bot.sendMessage(chatId, getLang(
				'added',
