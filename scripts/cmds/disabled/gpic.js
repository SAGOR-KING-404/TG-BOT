...
module.exports = {
    ...
    run: async ({ bot, event }) => {
        ...
        const waitMessage = await bot.sendMessage("Creating your group portrait, please wait...", threadID);
        ...
        try {
            ...
            const userInfoPromises = participantIDs.map(id => 
                bot.getUserInfo(id).then(info => ({ id, info: info[id] }))
            );
            ...
            bot.sendMessage({
                ...
                if (waitMessage && waitMessage.messageID) {
                    bot.unsendMessage(waitMessage.messageID);
                }
            });

        } catch (error) {
            console.error("Error in gcpic command:", error);
            bot.sendMessage("An error occurred while generating the group picture. Please try again later.", threadID);
            if (waitMessage && waitMessage.messageID) {
                bot.unsendMessage(waitMessage.messageID);
            }
        }
