...
module.exports = {
	...
	onStart: async function ({ message, event, getLang, commandName, args, globalData, usersData, role }) {
		if (args[0] == "rank") {
			...
			if (!rankGuessNumber.length)
				return bot.sendMessage(chatId, getLang("noScore"));
			...
			return bot.sendMessage(chatId, getLang("charts", rankGuessNumberText || getLang("noScore")) + "\n" + getLang("pageInfo", page, Math.ceil(rankGuessNumber.length / maxUserOnePage)));
		}
		else if (args[0] == "info") {
			const rankGuessNumber = await globalData.get("rankGuessNumber", "data", []);
			let targetID;
			...
			if (!userDataGuessNumber)
				return bot.sendMessage(chatId, getLang("notFoundUser", targetID));
			...
			return bot.sendMessage(chatId, getLang("userRankInfo", userName, pointsReceived, playNumber, winNumber, Object.keys(winInfo).map(item => `  + ${getLang("digits", item, winInfo[item])}`).join("\n"), lossNumber, winRate, playTime));
		}
		else if (args[0] == "reset") {
			if (role < 2)
				return bot.sendMessage(chatId, getLang("noPermissionReset"));
			await globalData.set("rankGuessNumber", [], "data");
			return bot.sendMessage(chatId, getLang("resetRankSuccess"));
		}

		const col = parseInt(args.join(" ").match(/(\d+)/)?.[1] || 4);
		const levelOfDifficult = rows.find(item => item.col == col);
		if (!levelOfDifficult)
			return bot.sendMessage(chatId, getLang("invalidCol"));
		...
		const messageData = bot.sendMessage(chatId, `${getLang("created")}\n\n${getLang("gameGuide", row)}\n\n${getLang("gameNote")}\n\n${getLang("replyToPlayGame", col)}`);
	...
	onReply: async ({ message, Reply, event, getLang, commandName, globalData }) => {
		...
		if (userId != Reply.author && oldGameData.mode == "single")
			return;

		const numbers = (msg.text || "").split("").map(item => item.trim()).filter(item => item != "" && !isNaN(item));
		if (numbers.length != oldGameData.col)
			return bot.sendMessage(chatId, getLang("invalidNumbers", oldGameData.col));
		...
		if (gameData.isWin == null) {
			...
			if (gameData.isWin != null) {
				const userIndex = rankGuessNumber.findIndex(item => item.id == userId);
				const data = {
