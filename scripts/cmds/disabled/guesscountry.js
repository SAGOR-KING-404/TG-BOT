...
async function onCall({ message, args }) {
  ...
  const waitMsg = await bot.sendMessage(chatId, `🔍 Guessing country for "${query}", please wait...`);

  try {
    const apiUrl = `https://sus-apis.onrender.com/bot/guess-country?input=${encodeURIComponent(query)}`;
    ...
    await bot.sendMessage(chatId, "❌ Failed to guess the country. Maybe invalid input or API error.");
  } finally {
