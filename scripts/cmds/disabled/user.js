...
async function onCall({ message, args, getLang, data }) {
    try {
        ...
        if (!targetIDs) return bot.sendMessage(chatId, getLang("missingTarget"));

        const members = data?.thread?.info?.members;
        if (!members) return bot.sendMessage(chatId, getLang("noData"));
        ...
        console.error(e);
        bot.sendMessage(chatId, getLang("error"));
    }
}
