...
module.exports = {
	...
	onStart: async function ({ message, event, threadsData, args, getLang }) {
		...
		await threadsData.set(chatId, args[0] == "on", "settings.sendRankupMessage");
		return bot.sendMessage(chatId, args[0] == "on" ? getLang("turnedOn") : getLang("turnedOff"));
	},
	...
	onChat: async function ({ threadsData, usersData, event, message, getLang }) {
		...
		if (currentLevel > expToLevel(exp - 1)) {
			let customMessage = await threadsData.get(chatId, "data.rankup.message");
			let isTag = false;
			let userData;
			const formMessage = {};
