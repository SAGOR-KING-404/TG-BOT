...
module.exports = {
	...
	onStart: async function ({ args, message, event, usersData, threadsData, bot, commandName, getLang }) {
		const { config } = global.GoatBot;
		if (!args[0])
			return bot.sendMessage(chatId, getLang("missingMessage"));
		const { senderID, threadID, isGroup } = event;
		if (config.adminBot.length == 0)
			return bot.sendMessage(chatId, getLang("noAdmin"));
		...
		for (const uid of config.adminBot) {
			try {
				const messageSend = await bot.sendMessage(formMessage, uid);
				successIDs.push(uid);
				global.GoatBot.onReply.set(messageSend.messageID, {
					commandName,
					messageID: messageSend.messageID,
					threadID,
					messageIDSender: msg.message_id,
					type: "userCallAdmin"
				});
			}
			catch (err) {
	...
	onReply: async ({ args, event, bot, message, Reply, usersData, commandName, getLang }) => {
		...
		switch (type) {
			case "userCallAdmin": {
				...
				bot.sendMessage(formMessage, threadID, (err, info) => {
					if (err)
						return message.err(err);
					bot.sendMessage(chatId, getLang("replyUserSuccess"));
			...
			case "adminReply": {
				let sendByGroup = "";
				if (isGroup) {
					const { threadName } = await bot.getThreadInfo(chatId);
					sendByGroup = getLang("sendByGroup", threadName, chatId);
				}
				const formMessage = {
					body: getLang("feedback", senderName, userId, sendByGroup, args.join(" ")),
					mentions: [{
						id: userId,
						tag: senderName
					}],
				...
				bot.sendMessage(formMessage, threadID, (err, info) => {
					if (err)
						return message.err(err);
					bot.sendMessage(chatId, getLang("replySuccess"));
					global.GoatBot.onReply.set(info.messageID, {
						commandName,
						messageID: info.messageID,
						messageIDSender: msg.message_id,
