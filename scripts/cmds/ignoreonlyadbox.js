module.exports = {
	...
	onStart: async function ({ args, message, threadsData, getLang, event }) {
		const ignoreList = await threadsData.get(chatId, "data.ignoreCommanToOnlyAdminBox", []);
		switch (args[0]) {
			case "add": {
				if (!args[1])
					return bot.sendMessage(chatId, getLang("missingCommandNameToAdd"));
				const commandName = args[1].toLowerCase();
				const command = global.GoatBot.commands.get(commandName);
				if (!command)
					return bot.sendMessage(chatId, getLang("commandNotFound", commandName));
				if (ignoreList.includes(commandName))
					return bot.sendMessage(chatId, getLang("commandAlreadyInList", commandName));
				ignoreList.push(commandName);
				await threadsData.set(chatId, ignoreList, "data.ignoreCommanToOnlyAdminBox");
				return bot.sendMessage(chatId, getLang("commandAdded", commandName));
			}
			case "del":
			case "delete":
			case "remove":
			...
			case "-d": {
				if (!args[1])
					return bot.sendMessage(chatId, getLang("missingCommandNameToDelete"));
				const commandName = args[1].toLowerCase();
				const command = global.GoatBot.commands.get(commandName);
				if (!command)
					return bot.sendMessage(chatId, getLang("commandNotFound", commandName));
				...
				if (!ignoreList.includes(commandName))
					return bot.sendMessage(chatId, getLang("commandNotInList", commandName));
				ignoreList.splice(ignoreList.indexOf(commandName), 1);
				await threadsData.set(chatId, ignoreList, "data.ignoreCommanToOnlyAdminBox");
				return bot.sendMessage(chatId, getLang("commandDeleted", commandName));
			}
			case "list": {
				return bot.sendMessage(chatId, getLang("ignoreList", ignoreList.join(", ")));
			}
			default: {
