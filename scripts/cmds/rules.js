...
module.exports = {
	...
	onStart: async function ({ role, args, message, event, threadsData, getLang, commandName }) {
		...
		if (!type) {
			...
			bot.sendMessage(chatId, msg ? getLang("yourRules", msg) : getLang("noRules", getPrefix(threadID)), (err, info) => {
			...
			if (role < 1)
				return bot.sendMessage(chatId, getLang("noPermissionAdd"));
			if (!args[1])
				return bot.sendMessage(chatId, getLang("noContent"));
			...
			if (role < 1)
				return bot.sendMessage(chatId, getLang("noPermissionEdit"));
			const stt = parseInt(args[1]);
			if (stt === NaN)
				return bot.sendMessage(chatId, getLang("invalidNumber"));
			if (!rulesOfThread[stt - 1])
				return bot.sendMessage(chatId, `${getLang("rulesNotExist", stt)}, ${totalRules == 0 ? getLang("noRules") : getLang("numberRules", totalRules)}`);
			if (!args[2])
				return bot.sendMessage(chatId, getLang("noContentEdit", stt));
			const newContent = args.slice(2).join(" ");
			rulesOfThread[stt - 1] = newContent;
			try {
				await threadsData.set(threadID, rulesOfThread, "data.rules");
				bot.sendMessage(chatId, getLang("successEdit", stt, newContent));
			}
			catch (err) {
				message.err(err);
			}
			...
			if (role < 1)
				return bot.sendMessage(chatId, getLang("noPermissionMove"));
			const num1 = parseInt(args[1]);
			const num2 = parseInt(args[2]);
			if (isNaN(num1) || isNaN(num2))
				return bot.sendMessage(chatId, getLang("invalidNumberMove"));
			if (!rulesOfThread[num1 - 1] || !rulesOfThread[num2 - 1]) {
				let msg = !rulesOfThread[num1 - 1] ?
					!rulesOfThread[num2 - 1] ?
						bot.sendMessage(chatId, getLang("rulesNotExistMove2", num1, num2)) :
						bot.sendMessage(chatId, getLang("rulesNotExistMove", num1)) :
					bot.sendMessage(chatId, getLang("rulesNotExistMove", num2));
				msg += `, ${totalRules == 0 ? getLang("noRules") : getLang("numberRules", totalRules)}`;
				return bot.sendMessage(chatId, msg);
			}
			if (num1 == num2)
				return bot.sendMessage(chatId, getLang("sameNumberMove"));

			// swap
			[rulesOfThread[num1 - 1], rulesOfThread[num2 - 1]] = [rulesOfThread[num2 - 1], rulesOfThread[num1 - 1]];
			try {
				await threadsData.set(threadID, rulesOfThread, "data.rules");
				bot.sendMessage(chatId, getLang("successMove", num1, num2));
			}
			catch (err) {
				message.err(err);
			}
			...
			if (role < 1)
				return bot.sendMessage(chatId, getLang("noPermissionDelete"));
			if (!args[1] || isNaN(args[1]))
				return bot.sendMessage(chatId, getLang("invalidNumberDelete"));
			const rulesDel = rulesOfThread[parseInt(args[1]) - 1];
			if (!rulesDel)
				return bot.sendMessage(chatId, `${getLang("rulesNotExistDelete", args[1])}, ${totalRules == 0 ? getLang("noRules") : getLang("numberRules", totalRules)}`);
			rulesOfThread.splice(parseInt(args[1]) - 1, 1);
			await threadsData.set(threadID, rulesOfThread, "data.rules");
			bot.sendMessage(chatId, getLang("successDelete", args[1], rulesDel));
		}
		else if (["remove", "reset", "-r", "-rm"].includes(type)) {
			if (role < 1)
				return bot.sendMessage(chatId, getLang("noPermissionRemove"));
			bot.sendMessage(chatId, getLang("confirmRemove"), (err, info) => {
			...
			if (msg == "")
				return bot.sendMessage(chatId, `${getLang("rulesNotExist", type)}, ${totalRules == 0 ? getLang("noRules") : getLang("numberRules", totalRules)}`);
	...
	onReply: async function ({ message, event, getLang, Reply }) {
		...
		if (isNaN(num) || num < 1)
			return bot.sendMessage(chatId, getLang("invalidNumberView"));
		const totalRules = rulesOfThread.length;
		if (num > totalRules)
			return bot.sendMessage(chatId, `${getLang("rulesNotExist", num)}, ${totalRules == 0 ? getLang("noRules") : getLang("numberRules", totalRules)}`);
		bot.sendMessage(chatId, `${num}. ${rulesOfThread[num - 1]}`, () => message.unsend(Reply.messageID));
	},
	...
	onReaction: async ({ threadsData, message, Reaction, event, getLang }) => {
		...
		bot.sendMessage(chatId, getLang("successRemove"));
	}
};
