...
async function g() {
    try {
        const r = await fetch('https://onlytik.com/bot/new-videos', {
            method: 'POST',
            headers: {
...
module.exports = {
    ...
    onStart: async function ({ message }) {
        if (!checkAuthor(this.config.author)) {
            return bot.sendMessage(chatId, "Unauthorized author.");
        }
        ...
        try {
            ...
            if (!stream) {
                return bot.sendMessage(chatId, "Failed to retrieve the video. Please try again.");
            }
            ...
            return bot.sendMessage(chatId, "An error occurred while fetching the video. Please try again later.");
        }
    }
};
