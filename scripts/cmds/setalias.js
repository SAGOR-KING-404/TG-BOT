module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		const aliasesData = await threadsData.get(chatId, "data.aliases", {});
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.GoatBot.commands.has(commandName))
					return bot.sendMessage(chatId, getLang("commandNotExist", commandName));
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (globalAliasesExist)
							return bot.sendMessage(chatId, getLang("aliasExist", alias, globalAliasesExist.commandName));
						if (global.GoatBot.aliases.has(alias))
							return bot.sendMessage(chatId, getLang("aliasExist", alias, global.GoatBot.aliases.get(alias)));
						...
						return bot.sendMessage(chatId, getLang("addAliasSuccess", alias, commandName));
					}
					else {
						return bot.sendMessage(chatId, getLang("noPermissionAdd", alias, commandName));
					}
				}

				if (global.GoatBot.commands.get(alias))
					return bot.sendMessage(chatId, getLang("aliasIsCommand", alias));
				if (global.GoatBot.aliases.has(alias))
					return bot.sendMessage(chatId, getLang("aliasExist", alias, global.GoatBot.aliases.get(alias)));
				for (const cmdName in aliasesData)
					if (aliasesData[cmdName].includes(alias))
						return bot.sendMessage(chatId, getLang("aliasExistInGroup", alias, cmdName));

				const oldAlias = aliasesData[commandName] || [];
				oldAlias.push(alias);
				aliasesData[commandName] = oldAlias;
				await threadsData.set(chatId, aliasesData, "data.aliases");
				return bot.sendMessage(chatId, getLang("addAliasToGroupSuccess", alias, commandName));
			}
			case "remove":
			case "rm": {
				if (!args[2])
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (!globalAliasesThisCommand || !globalAliasesThisCommand.aliases.includes(alias))
							return bot.sendMessage(chatId, getLang("aliasNotExist", alias, commandName));
						...
						return bot.sendMessage(chatId, getLang("removeAliasSuccess", alias, commandName));
					}
					else {
						return bot.sendMessage(chatId, getLang("noPermissionDelete", alias, commandName));
					}
				}

				const oldAlias = aliasesData[commandName];
				if (!oldAlias)
					return bot.sendMessage(chatId, getLang("noAliasInGroup", commandName));
				const index = oldAlias.indexOf(alias);
				if (index === -1)
					return bot.sendMessage(chatId, getLang("aliasNotExist", alias, commandName));
				oldAlias.splice(index, 1);
				await threadsData.set(chatId, aliasesData, "data.aliases");
				return bot.sendMessage(chatId, getLang("removeAliasInGroupSuccess", alias, commandName));
			}
			case "list": {
				if (args[1] == '-g') {
					const globalAliasesData = await globalData.get('setalias', 'data', []);
				...
				if (!Object.keys(aliasesData).length)
					return bot.sendMessage(chatId, getLang("notExistAliasInGroup"));
				const list = Object.keys(aliasesData).map(commandName => `\n• ${commandName}: ${aliasesData[commandName].join(", ")} `);
				return bot.sendMessage(chatId, getLang("aliasListInGroup", list.join("\n")));
			}
			default: {
				return message.SyntaxError();
			}
