...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang }) {
		...
		async function checkAndSaveData(key, data) {
			...
			bot.sendMessage(chatId, getLang(`antiChange${key.slice(0, 1).toUpperCase()}${key.slice(1)}${args[1].slice(0, 1).toUpperCase()}${args[1].slice(1)}`));
		}
		switch (args[0]) {
			case "avt":
			case "avatar":
			case "image": {
				...
				if (!imageSrc)
					return bot.sendMessage(chatId, getLang("missingAvt"));
	...
	onEvent: async function ({ message, event, threadsData, role, bot, getLang }) {
		...
		switch (logMessageType) {
			case "log:thread-image": {
				...
				return async function () {
					...
					if (role < 1 && bot.getCurrentUserID() !== author) {
						if (dataAntiChange.avatar != "REMOVE") {
							bot.sendMessage(chatId, getLang("antiChangeAvatarAlreadyOn"));
							bot.changeGroupImage(await getStreamFromURL(dataAntiChange.avatar), threadID);
						}
						else {
							bot.sendMessage(chatId, getLang("antiChangeAvatarAlreadyOnButMissingAvt"));
						}
					}
			...
			case "log:thread-name": {
				...
				return async function () {
					if (role < 1 && bot.getCurrentUserID() !== author) {
						bot.sendMessage(chatId, getLang("antiChangeNameAlreadyOn"));
						bot.setTitle(dataAntiChange.name, threadID);
					}
					else {
			...
			case "log:user-nickname": {
				...
				return async function () {
					...
					if (role < 1 && bot.getCurrentUserID() !== author) {
						bot.sendMessage(chatId, getLang("antiChangeNicknameAlreadyOn"));
						bot.changeNickname(dataAntiChange.nickname[participant_id], threadID, participant_id);
					}
					else {
			...
			case "log:thread-color": {
				...
				return async function () {
					if (role < 1 && bot.getCurrentUserID() !== author) {
						bot.sendMessage(chatId, getLang("antiChangeThemeAlreadyOn"));
						bot.changeThreadColor(dataAntiChange.theme || "196241301102133", threadID); // 196241301102133 is default color
					}
					else {
			...
			case "log:thread-icon": {
				...
				return async function () {
					if (role < 1 && bot.getCurrentUserID() !== author) {
						bot.sendMessage(chatId, getLang("antiChangeEmojiAlreadyOn"));
						bot.changeThreadEmoji(dataAntiChange.emoji, threadID);
					}
					else {
