...
module.exports = {
	...
	onStart: async function ({ args, threadsData, message, event, commandName, getLang }) {
		...
		switch (args[0]) {
			case "text": {
				...
				bot.sendMessage(chatId, data.welcomeMessage ? getLang("edited", data.welcomeMessage) : getLang("reseted"));
				break;
			}
			case "file": {
				if (args[1] == "reset") {
					...
					if (!welcomeAttachment)
						return bot.sendMessage(chatId, getLang("noFile"));
					try {
					...
					bot.sendMessage(chatId, getLang("resetedFile"));
				}
				else if (event.attachments.length == 0 && (!event.messageReply || event.messageReply.attachments.length == 0))
					return bot.sendMessage(chatId, getLang("missingFile"), (err, info) => {
			...
			case "off": {
				...
				bot.sendMessage(chatId, settings.sendWelcomeMessage ? getLang("turnedOn") : getLang("turnedOff"));
				break;
			}
			default:
				message.SyntaxError();
	...
	onReply: async function ({ event, Reply, message, threadsData, getLang }) {
		...
		if (event.attachments.length == 0 && (!event.messageReply || event.messageReply.attachments.length == 0))
			return bot.sendMessage(chatId, getLang("missingFile"));
		saveChanges(message, event, threadID, senderID, threadsData, getLang);
	}
};
...
async function saveChanges(message, event, threadID, senderID, threadsData, getLang) {
	...
	bot.sendMessage(chatId, getLang("addedFile", attachments.length));
}
