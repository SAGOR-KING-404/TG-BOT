    ...
    switch (args[0]) {
      ...
      case "-a": {
        if (args[1]) {
          ...
          return bot.sendMessage(chatId, getLang("missingIdAdd"));
      }
      case "remove":
      case "-r": {
        if (args[1]) {
          ...
          return bot.sendMessage(chatId, getLang("missingIdRemove"));
      }
      case "list":
      case "-l": {
        const getNames = await Promise.all(config.whiteListMode.whiteListIds.map(uid => usersData.getName(uid).then(name => ({ uid, name }))));
        return bot.sendMessage(chatId, getLang("listAdmin", getNames.map(({ uid, name }) => `• ${name} (${uid})`).join("\n")));
      }
      case "enable":
      case "on": {
        config.whiteListMode.enable = true;
        writeFileSync(global.client.dirConfig, JSON.stringify(config, null, 2));
        return bot.sendMessage(chatId, getLang("whiteListModeEnable"));
      }
      case "disable":
      case "off": {
        config.whiteListMode.enable = false;
        writeFileSync(global.client.dirConfig, JSON.stringify(config, null, 2));
        return bot.sendMessage(chatId, getLang("whiteListModeDisable"));
      }
      default:
