module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, usersData, role, getLang }) {
		if (!await threadsData.get(chatId, "data.badWords"))
			await threadsData.set(chatId, {
				words: [],
				violationUsers: {}
			}, "data.badWords");

		const badWords = await threadsData.get(chatId, "data.badWords.words", []);
		...
		switch (args[0]) {
			case "add": {
				...
				if (words.length === 0)
					return bot.sendMessage(chatId, getLang("missingWords"));
				const badWordsExist = [];
				const success = [];
				const failed = [];
				...
				await threadsData.set(chatId, badWords, "data.badWords.words");
			...
			case "-d": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("onlyAdmin2"));
				const words = args.slice(1).join(" ").split(/[,|]/);
				if (words.length === 0)
					return bot.sendMessage(chatId, getLang("missingWords2"));
				const success = [];
				const failed = [];
				for (const word of words) {
					const oldIndex = badWords.indexOf(word);
			...
			case "-a": {
				if (badWords.length === 0)
					return bot.sendMessage(chatId, getLang("emptyList"));
				bot.sendMessage(chatId, getLang("badWordsList", args[1] === "hide" ? badWords.map(word => hideWord(word)).join(", ") : badWords.join(", ")));
				break;
			}
			case "on": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("onlyAdmin3", getLang("onText")));
				await threadsData.set(chatId, true, "settings.badWords");
				bot.sendMessage(chatId, getLang("turnedOnOrOff", getLang("onText")));
				break;
			}
			case "off": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("onlyAdmin3", getLang("offText")));
				await threadsData.set(chatId, false, "settings.badWords");
				bot.sendMessage(chatId, getLang("turnedOnOrOff", getLang("offText")));
				break;
			}
			case "unwarn": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("onlyAdmin4"));
				let userID;
				...
				if (isNaN(userID))
					return bot.sendMessage(chatId, getLang("missingTarget"));
				const violationUsers = await threadsData.get(chatId, "data.badWords.violationUsers", {});
				if (!violationUsers[userID])
					return bot.sendMessage(chatId, getLang("notWarned", userID));
				violationUsers[userID]--;
				await threadsData.set(chatId, violationUsers, "data.badWords.violationUsers");
				const userName = await usersData.getName(userID);
				bot.sendMessage(chatId, getLang("unwarned", userID, userName));
			}
		}
	},

	onChat: async function ({ message, event, bot, threadsData, prefix, getLang }) {
		if (!msg.text)
			return;
		const threadData = global.db.allThreadData.find(t => t.threadID === chatId) || await threadsData.create(chatId);
		...
		const isCommand = allAliases.some(a => msg.text.startsWith(prefix + a));
		if (isCommand)
			return;
		...
		for (const word of badWordList) {
			if (msg.text.match(new RegExp(`\\b${word}\\b`, "gi"))) {
				if ((violationUsers[userId] || 0) < 1) {
					bot.sendMessage(chatId, getLang("warned", word));
					violationUsers[userId] = violationUsers[userId] ? violationUsers[userId] + 1 : 1;
					await threadsData.set(chatId, violationUsers, "data.badWords.violationUsers");
					return;
				}
				else {
					await bot.sendMessage(chatId, getLang("warned2", word));
					bot.removeUserFromGroup(userId, chatId, (err) => {
						if (err)
							return bot.sendMessage(chatId, getLang("needAdmin"), (e, info) => {
									...
									onStart: ({ event }) => {
										if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
											...
											if (TARGET_ID == bot.getCurrentUserID())
												bot.removeUserFromGroup(userId, chatId, () => onEvent = onEvent.filter(item => item.messageID != info.messageID));
										}
									}
								});
							});
