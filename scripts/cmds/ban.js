...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang, usersData, bot }) {
		const { members, adminIDs } = await threadsData.get(chatId);
		const { senderID } = event;
		let target;
		let reason;

		const dataBanned = await threadsData.get(chatId, 'data.banned_ban', []);
		...
		if (args[0] == 'unban') {
			if (!isNaN(args[1]))
				...
				return bot.sendMessage(getLang('notFoundTargetUnban'), chatId, msg.message_id);

			const index = dataBanned.findIndex(item => item.id == target);
			if (index == -1)
				return bot.sendMessage(getLang('userNotBanned', target), chatId, msg.message_id);

			dataBanned.splice(index, 1);
			await threadsData.set(chatId, dataBanned, 'data.banned_ban');
			const userName = members[target]?.name || await usersData.getName(target) || getLang('noName');

			return bot.sendMessage(getLang('unbannedSuccess', userName), chatId, msg.message_id);
		}
			...
			for (const user of dataBanned) {
				if (event.participantIDs.includes(user.id))
					bot.removeUserFromGroup(user.id, chatId);
			}
		}
		...
		if (event.messageReply?.senderID) {
			...
			if (!dataBanned.length)
				return bot.sendMessage(chatId, getLang('noData'));
			const limit = 20;
			...
			return bot.sendMessage(chatId, getLang('listBanned', page, Math.ceil(dataBanned.length / limit)) + '\n\n' + msg);
		}

		if (!target)
			return bot.sendMessage(chatId, getLang('notFoundTarget'));
		if (target == senderID)
			return bot.sendMessage(chatId, getLang('cantSelfBan'));
		if (adminIDs.includes(target))
			return bot.sendMessage(chatId, getLang('cantBanAdmin'));

		const banned = dataBanned.find(item => item.id == target);
		if (banned)
			return bot.sendMessage(chatId, getLang('existedBan'));
							...
							onStart: ({ event }) => {
								if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
									...
									if (TARGET_ID == bot.getCurrentUserID()) {
										bot.removeUserFromGroup(target, chatId, () => global.GoatBot.onEvent = global.GoatBot.onEvent.filter(item => item.messageID != info.messageID));
									}
								}
							}
						});
	...
	onEvent: async function ({ event, bot, threadsData, getLang, message }) {
		if (event.logMessageType == "log:subscribe") {
			...
			for (const user of usersAdded) {
				...
				if (banned) {
					...
					return bot.removeUserFromGroup(userFbId, threadID, err => {
						if (err)
								...
								global.GoatBot.onEvent.push({
									...
									onStart: ({ event }) => {
										if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
											const { TARGET_ID } = event.logMessageData;
											if (TARGET_ID == bot.getCurrentUserID()) {
												bot.removeUserFromGroup(userFbId, chatId, () => global.GoatBot.onEvent = global.GoatBot.onEvent.filter(item => item.messageID != info.messageID));
											}
										}
									}
								});
