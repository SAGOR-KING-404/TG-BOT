...
module.exports = {
	...
	onStart: async function ({ message, event, args, usersData, threadsData, bot, getLang, role }) {
		...
		switch (args[0]) {
			case "create": {
				...
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupName'));

				const groupsSendNotiData = await usersData.get(senderID, 'data.groupsSendNoti', []);
				if (groupsSendNotiData.some(item => item.groupName === groupName))
					return bot.sendMessage(chatId, getLang('groupNameExists', groupName));
				...
				bot.sendMessage(chatId, getLang('createdGroup', groupName, groupID));
				break;
			}
			case "add": {
				const groupName = args.slice(1).join(' ');
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupNameToAdd'));
				const getGroup = (groupsSendNotiData || []).find(item => item.groupName == groupName);

				if (!getGroup)
					return bot.sendMessage(chatId, getLang('groupNameNotExists', groupName));

				if (role < 1)
					return bot.sendMessage(chatId, getLang('notAdmin'));

				getGroup.threadIDs.push(threadID);
				await usersData.set(senderID, groupsSendNotiData, 'data.groupsSendNoti');

				bot.sendMessage(chatId, getLang('added', groupName));
				break;
			}
			case "list": {
				if (!groupsSendNotiData.length)
					return bot.sendMessage(chatId, getLang('noGroup'));
				...
				bot.sendMessage(chatId, getLang('showList', msg));
				break;
			}
			case "delete": {
				const groupName = args.slice(1).join(' ');
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupNameToDelete'));
				...
				if (findIndexThread == -1)
					return bot.sendMessage(chatId, getLang('notInGroup', groupName));

				getGroup.threadIDs.splice(findIndexThread, 1);
				await usersData.set(senderID, groupsSendNotiData, 'data.groupsSendNoti');

				bot.sendMessage(chatId, getLang('deleted', groupName));
				break;
			}
			case "remove":
			case "-r": {
				const groupName = args.slice(1).join(' ');
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupNameToRemove'));
				...
				bot.sendMessage(chatId, getLang('removed', groupName));
				break;
			}
			case "send": {
				const groupName = args.slice(1).join(' ').split('|')[0].trim();
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupNameToSend'));

				const getGroup = (groupsSendNotiData || []).find(item => item.groupName == groupName);
				if (!getGroup)
					return bot.sendMessage(chatId, getLang('groupNameNotExists', groupName));
				if (getGroup.threadIDs.length == 0)
					return bot.sendMessage(chatId, getLang('groupIsEmpty', groupName));
				...
				const msgSend = bot.sendMessage(chatId, getLang('sending', groupName, threadIDs.length));
				for (const tid of threadIDs) {
					...
					pendings.push(
						new Promise(async (resolve, reject) => {
							...
							bot.sendMessage(formSend, tid, (err) => {
								if (err)
									reject({
										...err,
										threadID: tid,
				...
				bot.unsendMessage((await msgSend).messageID);
				let msg = "";
				if (success.length)
			...
			case "info": {
				const groupName = args.slice(1).join(' ');
				if (!groupName)
					return bot.sendMessage(chatId, getLang('missingGroupNameToView'));
				...
				bot.sendMessage(chatId, getLang('groupInfo', groupName, getGroup.groupID, getTime(getGroup.groupID, 'DD/MM/YYYY HH:mm:ss'), msg ? getLang('groupInfoHasGroup', msg) : getLang('groupIsEmpty', groupName)));
				break;
			}
			default: {
