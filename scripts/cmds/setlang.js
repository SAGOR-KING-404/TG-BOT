	...
	onStart: async function ({ message, args, getLang, threadsData, role, event }) {
		...
		if (["-g", "-global", "all"].includes(args[1]?.toLowerCase())) {
			...
			if (!fs.existsSync(pathLanguageFile))
				return bot.sendMessage(chatId, getLang("langNotFound", langCode));
			...
			return bot.sendMessage(chatId, getLang("setLangForAll", langCode));
		}

		await threadsData.set(chatId, langCode, "data.lang");
		return bot.sendMessage(chatId, (global.GoatBot.commands.get("setlang")?.langs[langCode]?.setLangForCurrent || "Set default language for current chat: %1").replace("%1", langCode));
	}
};