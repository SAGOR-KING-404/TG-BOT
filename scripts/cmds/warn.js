...
module.exports = {
	...
	onStart: async function ({ message, bot, event, args, threadsData, usersData, prefix, role, getLang }) {
		if (!args[0])
		...
		switch (args[0]) {
			case "list": {
				...
				bot.sendMessage(chatId, msg.length ? getLang("list", msg.join("\n"), prefix) : getLang("listEmpty"));
				break;
			}
			case "listban": {
				...
				bot.sendMessage(chatId, result.length ? getLang("listBan", result.join("\n")) : getLang("listBanEmpty"));
				break;
			}
			case "check":
			case "info": {
				...
				if (!uids)
					return bot.sendMessage(chatId, getLang("invalidUid"));
			...
			case "unban": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("noPermission"));
				let uidUnban;
				if (Object.keys(event.mentions).length)
				...
				if (!uidUnban || isNaN(uidUnban))
					return bot.sendMessage(chatId, getLang("invalidUid2"));

				const index = warnList.findIndex(user => user.uid == uidUnban && user.list.length >= 3);
				if (index === -1)
					return bot.sendMessage(chatId, getLang("notBanned", uidUnban));

				warnList.splice(index, 1);
				await threadsData.set(threadID, warnList, "data.warn");
				const userName = await usersData.getName(uidUnban);
				bot.sendMessage(chatId, getLang("unbanSuccess", uidUnban, userName));
				break;
			}
			case "unwarn": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("noPermission2"));
				let uid, num;
				...
				if (isNaN(uid))
					return bot.sendMessage(chatId, getLang("invalidUid3"));

				const dataWarnOfUser = warnList.find(u => u.uid == uid);
				if (!dataWarnOfUser?.list.length)
					return bot.sendMessage(chatId, getLang("noData2", uid));
				...
				if (num > dataWarnOfUser.list.length)
					return bot.sendMessage(chatId, getLang("notEnoughWarn", userName, dataWarnOfUser.list.length));

				dataWarnOfUser.list.splice(parseInt(num), 1);
				if (!dataWarnOfUser.list.length)
					warnList.splice(warnList.findIndex(u => u.uid == uid), 1);
				await threadsData.set(threadID, warnList, "data.warn");
				bot.sendMessage(chatId, getLang("unwarnSuccess", num + 1, uid, userName));
				break;
			}
			case "reset": {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("noPermission3"));
				await threadsData.set(threadID, [], "data.warn");
				bot.sendMessage(chatId, getLang("resetWarnSuccess"));
				break;
			}
			default: {
				if (role < 1)
					return bot.sendMessage(chatId, getLang("noPermission4"));
				let reason, uid;
				if (event.messageReply) {
					...
					return bot.sendMessage(chatId, getLang("invalidUid4"));
				}
				if (!reason)
					reason = "No reason";
				...
				const userName = await usersData.getName(uid);
				if (times >= 3) {
					bot.sendMessage(chatId, getLang("warnSuccess", userName, times, uid, reason, dateTime, prefix), () => {
						bot.removeUserFromGroup(uid, threadID, async (err) => {
							if (err) {
								const members = await threadsData.get(chatId, "members");
								if (members.find(item => item.userID == uid)?.inGroup) // check if user is still in group
									return bot.sendMessage(chatId, getLang("userNotInGroup", userName));
								else
									return bot.sendMessage(chatId, getLang("noPermission5"), (e, info) => {
										...
										onEvent.push({
											...
											onStart: async ({ event }) => {
												if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
													...
													if (TARGET_ID == bot.getCurrentUserID()) {
														const warnList = await threadsData.get(chatId, "data.warn", []);
														if ((warnList.find(user => user.uid == uid)?.list.length ?? 0) <= 3)
															...
															bot.removeUserFromGroup(uid, chatId, () => global.GoatBot.onEvent = onEvent.filter(item => item.messageID != info.messageID));
													}
												}
											}
										});
					...
					bot.sendMessage(chatId, getLang("warnSuccess2", userName, times, uid, reason, dateTime, 3 - (times)));
			}
		}
	},

	onEvent: async ({ event, threadsData, usersData, message, bot, getLang }) => {
		const { logMessageType, logMessageData } = event;
		if (logMessageType === "log:subscribe") {
			return async () => {
				const { data, adminIDs } = await threadsData.get(chatId);
				...
				if (hasBanned.length) {
					...
					if (!adminIDs.includes(bot.getCurrentUserID()))
						bot.sendMessage(chatId, getLang("noPermission5"), (e, info) => {
							const { onEvent } = global.GoatBot;
							onEvent.push({
								messageID: info.messageID,
								onStart: async ({ event }) => {
									if (
										...
										&& event.logMessageData.TARGET_ID == bot.getCurrentUserID()
									) {
										const threadData = await threadsData.get(chatId);
										const warnList = threadData.data.warn;
										const members = threadData.members;
										removeUsers(hasBanned, warnList, bot, event, message, getLang, members);
										global.GoatBot.onEvent = onEvent.filter(item => item.messageID != info.messageID);
									}
								}
							});
...
async function removeUsers(hasBanned, warnList, bot, event, message, getLang, members) {
	const failed = [];
		...
		if (members.find(item => item.userID == user.uid)?.inGroup) { // check if user is still in group
			try {
				if (warnList.find(item => item.uid == user.uid)?.list.length ?? 0 >= 3)
					await bot.removeUserFromGroup(user.uid, chatId);
			}
			catch (e) {
				failed.push({
					uid: user.uid,
	...
	if (failed.length)
		bot.sendMessage(chatId, getLang("failedKick", failed.map(item => `  - ${item.name} (uid: ${item.uid})`).join("\n")));
}