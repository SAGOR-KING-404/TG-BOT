...
module.exports = {
	...
	onStart: async function ({ args, message, getLang }) {
		...
		bot.sendMessage(chatId, getLang("initImage"));
		try {
			...
			if (!isNaN(content[0])) {
				...
				if (idNhanVat > totalCharacter)
					return bot.sendMessage(chatId, getLang("invalidCharacter", totalCharacter));
				...
				if (findChracter) {
					...
					return bot.sendMessage(chatId, getLang("notFoundCharacter", content[0]));
			}
		}
		catch (error) {
			const err = error.response.data;
			return bot.sendMessage(chatId, getLang("errorGetCharacter", err.error, err.message));
		}
		...
		try {
			...
			error.response.data.on("data", function (e) {
				...
				bot.sendMessage(chatId, getLang("error", err.error, err.message));
			});
		}
	}
};