...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang, commandName }) {
		if (["-r", "-react", "-reaction"].includes(args[0])) {
			if (args[1] == "set") {
				return bot.sendMessage(chatId, getLang("inputEmoji"), (err, info) =>
			...
			if (isEnable == null)
				return bot.sendMessage(chatId, getLang("invalidArgument"));
			await threadsData.set(chatId, isEnable, "data.translate.autoTranslateWhenReaction");
			return bot.sendMessage(chatId, isEnable ? getLang("turnOnTransWhenReaction") : getLang("turnOffTransWhenReaction"));
		}
		const { body = "" } = event;
		let content;
		let langCodeTrans;
		const langOfThread = await threadsData.get(chatId, "data.lang") || global.GoatBot.config.language;
	...
	onChat: async ({ event, threadsData }) => {
		if (!await threadsData.get(chatId, "data.translate.autoTranslateWhenReaction"))
			return;
		global.GoatBot.onReaction.set(msg.message_id, {
	...
	onReaction: async ({ message, Reaction, event, threadsData, getLang }) => {
		switch (Reaction.type) {
			case "setEmoji": {
				...
				if (!emoji)
					return;
				await threadsData.set(chatId, emoji, "data.translate.emojiTranslate");
				return bot.sendMessage(chatId, getLang("emojiSet", emoji), () => message.unsend(Reaction.messageID));
			}
			case "translate": {
				const emojiTrans = await threadsData.get(chatId, "data.translate.emojiTranslate") || "🌐";
				if (event.reaction == emojiTrans) {
					const langCodeTrans = await threadsData.get(chatId, "data.lang") || global.GoatBot.config.language;
