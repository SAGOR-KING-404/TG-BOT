	...
	onStart: async function ({ args, message, event, threadsData, getLang }) {
		...
		switch (args[0]) {
			case "text": {
				...
				return bot.sendMessage(chatId, getLang("changedMessage", newContent));
			}
			case "file":
			case "image":
			case "mp3":
			case "video": {
				...
				if (!attachments.length && !(args[1] || '').match(checkUrlRegex))
					return bot.sendMessage(chatId, getLang("missingAttachment", attachments.length));
				...
				return bot.sendMessage(chatId, getLang("changedAttachment"));
			}
		}
	}
};
