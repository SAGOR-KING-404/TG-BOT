...
module.exports = {
	...
	onStart: async function ({ message, args, getLang, envGlobal }) {
		...
		if (!goatBotApi.isSetApiKey())
			return bot.sendMessage(chatId, getLang("missingGoatApiKey"));
		const prompt = args.join(" ");
		if (!prompt)
			return bot.sendMessage(chatId, getLang("syntaxError"));

		try {
			const { data: imageStream } = await goatBotApi.bot({
				url: "/image/mdjrny",
				method: "GET",
				params: {
					prompt,
			...
			return bot.sendMessage(chatId, getLang("error", err.data?.message || err.message));
		}
	}
};