...
module.exports = {
	...
	onStart: async function ({ message, event, args, threadsData, getLang }) {
		switch (args[0]) {
			...
			case "config": {
				if (args.length < 2)
					return bot.sendMessage(chatId, getLang("missingConfig"));
				const configAutoSetName = args.slice(1).join(" ");
				await threadsData.set(chatId, configAutoSetName, "data.autoSetName");
				return bot.sendMessage(chatId, getLang("configSuccess"));
			}
			case "view":
			case "info": {
				const configAutoSetName = await threadsData.get(chatId, "data.autoSetName");
				return bot.sendMessage(chatId, configAutoSetName ? getLang("currentConfig", configAutoSetName) : getLang("notSetConfig"));
			}
			default: {
				const enableOrDisable = args[0];
				if (enableOrDisable !== "on" && enableOrDisable !== "off")
					return bot.sendMessage(chatId, getLang("syntaxError"));
				await threadsData.set(chatId, enableOrDisable === "on", "settings.enableAutoSetName");
				return bot.sendMessage(chatId, enableOrDisable == "on" ? getLang("turnOnSuccess") : getLang("turnOffSuccess"));
			}
		}
	},

	onEvent: async ({ message, event, bot, threadsData, getLang }) => {
		if (event.logMessageType !== "log:subscribe")
			return;
		if (!await threadsData.get(chatId, "settings.enableAutoSetName"))
			return;
		const configAutoSetName = await threadsData.get(chatId, "data.autoSetName");
		...
		return async function () {
			...
			for (const user of addedParticipants) {
				...
				try {
					await bot.changeNickname(checkShortCut(configAutoSetName, uid, userName), chatId, uid);
				}
				catch (e) {
