module.exports = {
	...
	onStart: async function ({ message, event, args, role, threadsData, getLang }) {
		...
		const setRole = await threadsData.get(chatId, "data.setRole", {});

		if (["view", "viewrole", "show"].includes(args[0])) {
			if (!setRole || Object.keys(setRole).length === 0)
				return bot.sendMessage(chatId, getLang("noEditedCommand"));
		...
		if (command.config.role > 1)
			return bot.sendMessage(chatId, getLang("noChangeRole", commandName));
		...
		await threadsData.set(chatId, setRole, "data.setRole");
		bot.sendMessage(chatId, "✅ " + (Default === true ? getLang("resetRole", commandName) : getLang("changedRole", commandName, newRole)));
	}
};