...
module.exports = {
	...
	onStart: async function ({ message, event, commandName, getLang }) {
		const datagame = (await axios.get("https://goatbotserver.onrender.com/bot/duoihinhbatchu")).data;
	...
	onReply: async ({ message, Reply, event, getLang, usersData, envCommands, commandName }) => {
		...
		if (userId != author)
			return bot.sendMessage(chatId, getLang("notPlayer"));

		if (formatText(msg.text) == formatText(wordcomplete)) {
			global.GoatBot.onReply.delete(messageID);
			await usersData.addMoney(userId, envCommands[commandName].reward);
			bot.sendMessage(chatId, getLang("correct", envCommands[commandName].reward));
		}
		else
			bot.sendMessage(chatId, getLang("wrong"));
	}
};
