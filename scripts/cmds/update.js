...
module.exports = {
	...
	onLoad: async function ({ bot }) {
		if (fs.existsSync(dirBootLogTemp)) {
			...
			bot.sendMessage("The chatbot has been restarted.", threadID);
		}
	},
	...
	onStart: async function ({ message, getLang, commandName, event }) {
		...
		if (compareVersion(version, currentVersion) < 1)
			return bot.sendMessage(chatId, getLang("noUpdates", currentVersion));
	...
	onReaction: async function ({ message, getLang, Reaction, event, commandName }) {
		...
		const { data: lastCommit } = await axios.get('https://bot.github.com/repos/ntkhang03/Goat-Bot-V2/commits/main');
		...
		if (new Date().getTime() - lastCommitDate.getTime() < 5 * 60 * 1000) {
			...
			return bot.sendMessage(chatId, getLang("updateTooFast", minutes, seconds, minutesCooldown, secondsCooldown));
		}

		await bot.sendMessage(chatId, getLang("updateConfirmed"));
		// Update chatbot
		execSync("node update", {
			stdio: "inherit"
		});
		fs.writeFileSync(dirBootLogTemp, chatId);

		bot.sendMessage(chatId, getLang("updateComplete"), (err, info) => {
			if (err)
				return console.error(err);
	...
	onReply: async function ({ message, getLang, event }) {
		if (['yes', 'y'].includes(msg.text?.toLowerCase())) {
			await bot.sendMessage(chatId, getLang("botWillRestart"));
			process.exit(2);
		}
	}
};
