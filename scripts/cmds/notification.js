...
module.exports = {
	...
	onStart: async function ({ message, bot, event, args, commandName, envCommands, threadsData, getLang }) {
		...
		const allThreadID = (await threadsData.getAll()).filter(t => t.isGroup && t.members.find(m => m.userID == bot.getCurrentUserID())?.inGroup);
		bot.sendMessage(chatId, getLang("sendingNotification", allThreadID.length));
		...
		for (const thread of allThreadID) {
			...
			try {
				wattingSend.push({
					threadID: tid,
					pending: bot.sendMessage(formSend, tid)
				});
