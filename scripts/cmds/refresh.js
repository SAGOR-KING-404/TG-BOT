module.exports = {
	...
	onStart: async function ({ args, threadsData, message, event, usersData, getLang }) {
		if (args[0] == "group" || args[0] == "thread") {
			const targetID = args[1] || chatId;
			try {
				await threadsData.refreshInfo(targetID);
				return bot.sendMessage(chatId, targetID == chatId ? getLang("refreshMyThreadSuccess") : getLang("refreshThreadTargetSuccess", targetID));
			}
			catch (error) {
				return bot.sendMessage(chatId, targetID == chatId ? getLang("errorRefreshMyThread") : getLang("errorRefreshThreadTarget", targetID));
			}
		}
		else if (args[0] == "user") {
			let targetID = userId;
			...
			try {
				...
				return bot.sendMessage(chatId, targetID == userId ? getLang("refreshMyUserSuccess") : getLang("refreshUserTargetSuccess", targetID));
			}
			catch (error) {
				return bot.sendMessage(chatId, targetID == userId ? getLang("errorRefreshMyUser") : getLang("errorRefreshUserTarget", targetID));
			}
		}
		else
			message.SyntaxError();
