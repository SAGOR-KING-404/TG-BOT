...
module.exports = {
	...
	onStart: async function ({ args, message, threadModel, userModel, dashBoardModel, globalModel, getLang }) {
		if (global.GoatBot.config.database.type !== "mongodb")
			return bot.sendMessage(chatId, getLang("invalidDatabase"));
...
async function syncThreadData(message, threadModel, getLang) {
	...
	if (!fs.existsSync(pathThreadData))
		return bot.sendMessage(chatId, getLang("missingFile", pathThreadData.split("/").pop()));
	...
	try {
		...
		return bot.sendMessage(chatId, getLang("formatInvalid"));
	}
	...
	try {
		...
		return bot.sendMessage(chatId, getLang("successThread"));
	}
	catch (err) {
		return bot.sendMessage(chatId, getLang("error", err.name, err.message));
	}
}
...
async function syncUserData(message, userModel, getLang) {
	...
	if (!fs.existsSync(pathUsersData))
		return bot.sendMessage(chatId, getLang("missingFile", pathUsersData.split("/").pop()));
	...
	try {
		...
		return bot.sendMessage(chatId, getLang("successUser"));
	}
	catch (err) {
		return bot.sendMessage(chatId, getLang("error", err.name, err.message));
	}
...
async function syncDashBoardData(message, dashBoardModel, getLang) {
	...
	if (!fs.existsSync(pathDashBoardData))
		return bot.sendMessage(chatId, getLang("missingFile", pathDashBoardData.split("/").pop()));
	...
	try {
		...
		return bot.sendMessage(chatId, getLang("successDashboard"));
	}
	catch (err) {
		return bot.sendMessage(chatId, getLang("error", err.name, err.message));
	}
...
async function syncGlobalData(message, globalModel, getLang) {
	...
	if (!fs.existsSync(pathGlobalData))
		return bot.sendMessage(chatId, getLang("missingFile", pathGlobalData.split("/").pop()));
	...
	try {
		...
		return bot.sendMessage(chatId, getLang("successGlobal"));
	}
	catch (err) {
		return bot.sendMessage(chatId, getLang("error", err.name, err.message));
	}
