...
module.exports = {
	...
	onStart: async function ({ message, event, bot, args, getLang }) {
		...
		try {
			...
			return bot.sendMessage(chatId, getLang("cannotGetImage"));
		}
		if (!response.headers["content-type"].includes("image"))
			return bot.sendMessage(chatId, getLang("invalidImageFormat"));
		response.data.path = "avatar.jpg";

		bot.changeAvatar(response.data, caption, expirationAfter ? expirationAfter * 1000 : null, (err) => {
			if (err)
				return message.err(err);
			return bot.sendMessage(chatId, getLang("changedAvatar"));
		});
	}
};