...
module.exports = {
	...
	onStart: async function ({ bot, args, threadsData, message, event, commandName, getLang }) {
		const threadData = await threadsData.get(chatId);
		if (!threadData.adminIDs.includes(bot.getCurrentUserID()))
			return bot.sendMessage(chatId, getLang("needAdmin"));

		if (!isNaN(args[0])) {
			bot.sendMessage(chatId, getLang("confirm", args[0]), (err, info) => {
			...
			const threadData = await bot.getThreadInfo(chatId);
			...
			for (const user of membersBlocked) {
				if (user.type !== "User" && !threadData.adminIDs.some(id => id == user.id)) {
					try {
						await bot.removeUserFromGroup(user.id, chatId);
	...
	onReaction: async function ({ bot, Reaction, event, threadsData, message, getLang }) {
		const { minimum = 1, author } = Reaction;
		if (event.userID != author)
			return;
		const threadData = await threadsData.get(chatId);
		const botID = bot.getCurrentUserID();
		...
		for (const member of membersCountLess) {
			try {
				await bot.removeUserFromGroup(member.userID, chatId);
