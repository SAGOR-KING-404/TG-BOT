	...
	onStart: async function ({ args, threadsData, message, role, event, getLang }) {
		...
		switch (type) {
			...
			case "-b": {
				...
				if (!reason)
					return bot.sendMessage(chatId, getLang("missingReason"));
				...
				if (status)
					return bot.sendMessage(chatId, getLang("hasBanned", tid, name, threadData.banned.reason, threadData.banned.date));
				...
				return bot.sendMessage(chatId, getLang("banned", tid, name, reason, time));
			}
			// unban thread
			case "unban":
			case "-u": {
				...
				if (!status)
					return bot.sendMessage(chatId, getLang("notBanned", tid, name));
				await threadsData.set(tid, {
					banned: {}
				});
				return bot.sendMessage(chatId, getLang("unbanned", tid, name));
			}
			// info thread
			case "info":
			case "-i": {
