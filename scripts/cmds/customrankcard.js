...
module.exports = {
	...
	onStart: async function ({ message, threadsData, event, args, getLang, usersData, envCommands }) {
		...
		const customRankCard = await threadsData.get(chatId, "data.customRankCard", {});
		...
		if ([...notSupportImage, ...supportImage].includes(key)) {
			...
			if (value == 'reset') {
				...
				if (!matchUrl)
					return bot.sendMessage(chatId, getLang("invalidImage"));
				...
				if (!["photo", "animated_image"].includes(attachments[0].type))
					return bot.sendMessage(chatId, getLang("invalidAttachment"));
				...
				if (!colors)
					return bot.sendMessage(chatId, getLang("invalidColor"));
				value = colors.length == 1 ? colors[0] : colors;
			}

			if (value != "reset" && notSupportImage.includes(key) && value.startsWith?.("http"))
				return bot.sendMessage(chatId, getLang("notSupportImage", key));
			...
			try {
				await threadsData.set(chatId, customRankCard, "data.customRankCard");
				bot.sendMessage(chatId, {
					body: getLang("success"),
					attachment: await global.client.makeRankCard(userId, usersData, threadsData, chatId, envCommands["rank"]?.deltaNext || 5)
						.then(stream => {
			...
			if (parseFloat(value) < 0 && parseFloat(value) > 1)
				return bot.sendMessage(chatId, getLang("invalidAlpha"));
			...
			try {
				await threadsData.set(chatId, {}, "data.customRankCard");
				bot.sendMessage(chatId, getLang("reseted"));
			}
			catch (err) {
				message.err(err);
			}
