	...
	onStart: async function ({ message, event, args, threadsData, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!groupTagName)
					return bot.sendMessage(chatId, getLang("noGroupTagName"));
				if (mentionsID.length === 0)
					return bot.sendMessage(chatId, getLang("noMention"));
				...
				if (oldGroupTag) {
					...
					await threadsData.set(threadID, groupTags, "data.groupTags");
					bot.sendMessage(chatId, getLang("addedSuccess2", groupTagName, Object.values(mentions).join("\n")));
				}
				break;
			}
			case "list":
			case "all": {
				if (args[1]) {
					...
					if (!groupTag)
						return bot.sendMessage(chatId, getLang("noExistedGroupTag", groupTagName));
				...
				bot.sendMessage(chatId, msg || getLang("noExistedGroupTag2"));
				break;
			}
			case "info": {
				const groupTagName = args.slice(1).join(" ");
			...
			case "del": {
				...
				if (!groupTagName)
					return bot.sendMessage(chatId, getLang("noGroupTagName"));
				if (mentionsID.length === 0)
					return bot.sendMessage(chatId, getLang("noMention", groupTagName));
				const oldGroupTag = groupTags.find(tag => tag.name.toLowerCase() === groupTagName.toLowerCase());
				if (!oldGroupTag)
					return bot.sendMessage(chatId, getLang("noExistedGroupTag", groupTagName));
				const usersIDExist = [];
			...
			case "rm": {
				...
				await threadsData.set(threadID, groupTags, "data.groupTags");
				bot.sendMessage(chatId, getLang("deletedSuccess2", groupTagName));
				break;
			}
			case "rename": {
				const content = (args.slice(1) || []).join(" ");
				...
				if (!oldGroupTagName || !newGroupTagName)
					return bot.sendMessage(chatId, getLang("noGroupTagName2"));
				const oldGroupTag = groupTags.find(tag => tag.name.toLowerCase() === oldGroupTagName.toLowerCase());
				if (!oldGroupTag)
					return bot.sendMessage(chatId, getLang("noExistedGroupTag", oldGroupTagName));
				oldGroupTag.name = newGroupTagName;
				await threadsData.set(threadID, groupTags, "data.groupTags");
				bot.sendMessage(chatId, getLang("renamedSuccess", oldGroupTagName, newGroupTagName));
				break;
			}
			case "tag":
			default: {
...
function showInfoGroupTag(message, groupTag, getLang) {
	bot.sendMessage(chatId, getLang("infoGroupTag", groupTag.name, Object.keys(groupTag.users).length, Object.keys(groupTag.users).map(uid => groupTag.users[uid]).join("\n ")));
}