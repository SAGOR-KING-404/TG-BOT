module.exports = {
	...
	onStart: async function ({ args, threadsData, message, event, bot, commandName, getLang }) {
		const { threadID, senderID } = event;
		const threadData = await threadsData.get(threadID);
		const { members } = threadData;
		const usersInGroup = (await bot.getThreadInfo(threadID)).participantIDs;
		let arraySort = [];
		...
		if (args[0]) {
			if (args[0].toLowerCase() == "all") {
				...
				if ((msg + endMessage).length > 19999) {
					...
					return bot.sendMessage(chatId, msg, (err, info) => {
						if (err)
			...
			return bot.sendMessage(chatId, getLang("yourResult", findUser.stt, findUser.count));
		}
	},
	...
	onReply: ({ message, event, Reply, commandName, getLang }) => {
		...
		if (isNaN(page) || page < 1 || page > splitPage.totalPage)
			return bot.sendMessage(chatId, getLang("invalidPage"));
		let msg = getLang("count");
		const endMessage = getLang("endMessage");
		...
		msg += getLang("page", page, splitPage.totalPage)
			+ "\n" + getLang("reply")
			+ "\n\n" + endMessage;
		bot.sendMessage(chatId, msg, (err, info) => {
			if (err)
				return message.err(err);
