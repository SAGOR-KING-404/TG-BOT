...
3 ✦ Super Admins
  ...
  onStart: async function ({ message, role, args, commandName, event, threadsData, getLang }) {
    ...
    if (args[0].toLowerCase() === "reset") {
      await threadsData.set(chatId, null, "data.prefix");
      return bot.sendMessage(chatId, getLang("reset", global.GoatBot.config.prefix));
    }

    const newPrefix = args[0];
    const formSet = { commandName, author: userId, newPrefix };

    if (args[1] === "-g") {
      if (role < 2) return bot.sendMessage(chatId, getLang("onlyAdmin"));
  ...
  onReaction: async function ({ message, threadsData, event, Reaction, getLang }) {
    ...
    if (setGlobal) {
      ...
      return bot.sendMessage(chatId, getLang("successGlobal", newPrefix));
    } else {
      await threadsData.set(chatId, newPrefix, "data.prefix");
      return bot.sendMessage(chatId, getLang("successThisThread", newPrefix));
    }
  },

  onChat: async function ({ event, message, getLang }) {
    if (msg.text && msg.text.toLowerCase() === "prefix") {
      const sysPrefix = global.GoatBot.config.prefix;
      const threadPrefix = utils.getPrefix(chatId);
