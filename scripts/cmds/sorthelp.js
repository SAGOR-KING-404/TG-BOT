module.exports = {
	config: {
		name: "sorthelp",
		version: "1.2",
		author: "NTKhang",
		cooldown: 5,
		role: 0,
		description: {
			vi: "Sắp xếp danh sách help",
			en: "Sort help list"
		},
		category: "image",
		guide: {
			en: "{pn} [name | category]"
		}
	},

	langs: {
		vi: {
			savedName: "Đã lưu cài đặt sắp xếp danh sách help theo thứ tự chữ cái",
			savedCategory: "Đã lưu cài đặt sắp xếp danh sách help theo thứ tự thể loại"
		},
		en: {
			savedName: "Saved sort help list by name",
			savedCategory: "Saved sort help list by category"
		}
	},

	onStart: async function ({ message, event, args, threadsData, getLang }) {
		if (args[0] == "name") {
			await threadsData.set(chatId, "name", "settings.sortHelp");
			bot.sendMessage(chatId, getLang("savedName"));
		}
		else if (args[0] == "category") {
			threadsData.set(chatId, "category", "settings.sortHelp");
			bot.sendMessage(chatId, getLang("savedCategory"));
		}
		else
			message.SyntaxError();
	}
};