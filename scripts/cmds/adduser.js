...
module.exports = {
	...
	onStart: async function ({ message, bot, event, args, threadsData, getLang }) {
		const { members, adminIDs, approvalMode } = await threadsData.get(chatId);
		...
		for (const item of args) {
			...
			if (members.some(m => m.userID == uid && m.inGroup)) {
				...
				try {
					await bot.addUserToGroup(uid, chatId);
